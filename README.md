...
- **Использование:** при выборе `use_task_bank = true` система подбирает задачи из банка с использованием семантического поиска по эмбеддингам, исключая уже использованные и переиспользованные (>10 раз). Система учитывает направление, сложность, номер задачи и производительность кандидата для выбора наиболее подходящей задачи.
...
Система отслеживает следующие типы событий античита:

| Тип события (EN) | Название (RU) | Описание |
|------------------|---------------|----------|
| `tab_switch` | Переключение вкладки | Кандидат переключился на другую вкладку браузера |
| `copy_paste` | Копирование/вставка | Обнаружена операция копирования или вставки |
| `devtools_open` | Открытие DevTools | Открыты инструменты разработчика браузера |
| `focus_loss` | Потеря фокуса | Окно браузера потеряло фокус (переключение на другое приложение) |
| `large_paste` | Большая вставка | Вставлен большой объем текста (подозрение на копирование решения) |
| `suspicious_typing` | Подозрительный паттерн печати | Обнаружен необычный паттерн печати (слишком быстрая/медленная, без ошибок) |
| `code_change_timestamp` | Изменение кода | Зафиксировано изменение кода (для анализа временных меток) |
| `large_code_change` | Большое изменение кода | Внесено большое изменение в код за короткое время |
| `external_service_request` | Запрос к внешнему сервису | Обнаружен запрос к внешнему сервису (подозрение на использование API) |
| `ai_service_request` | Запрос к AI-сервису | Обнаружен запрос к AI-сервису (ChatGPT, Copilot и т.д.) |
| `call_service_request` | Запрос к сервису звонков | Обнаружен запрос к сервису видеозвонков (подозрение на помощь извне) |
| `frequent_paste` | Частая вставка | Множественные операции вставки за короткое время |
| `code_paste` | Вставка кода | Вставлен код (определяется через анализ содержимого) |

Все события сохраняются в таблице `anti_cheat_events` с метаданными (severity, details, timestamp) и агрегируются в `anti_cheat_metrics` для финального отчета.
...
- **Банк задач:** Задачи с `times_used > 10` исключаются из автоматического выбора. Эмбеддинги генерируются при добавлении задачи для семантического поиска. Система использует cosine similarity для выбора наиболее подходящих задач на основе контекста интервью (направление, сложность, производительность кандидата).
